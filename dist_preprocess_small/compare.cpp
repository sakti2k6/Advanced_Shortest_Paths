...
void ContractionHierarchies::printEdges () {
    for (int i = 0; i < G.adj[0].size(); ++i) {
        cout << i << " :: " ;
        for (int j = 0; j < G.adj[0][i].size(); ++j) 
            cout << i << "->" << G.adj[0][i][j].second << ":" << G.adj[0][i][j].first << " , ";
        cout << "\n";
    }
}

void ContractionHierarchies::printRanks() {
    for (int i = 0; i < rank.size(); ++i) {
        cout << i << ": " << rank[i] << endl;
    }
}

void ContractionHierarchies::printGraphStatistics() {
    int osum = 0;
    int isum = 0;
    for (const auto& e: G.adj[0]) {
        osum += e.size();
    }
    for (const auto& e: G.adj[1]) {
        isum += e.size();
    }
