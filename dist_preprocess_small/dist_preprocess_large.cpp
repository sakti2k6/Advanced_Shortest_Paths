...
/*##########################################*/
    ...
    int get_importance (int i) {
        //return incoming_edges[i].size() * outgoing_edges[i].size();
        int imp = (incoming_edges[i].size() * outgoing_edges[i].size() - incoming_edges[i].size() - outgoing_edges[i].size()) + 1 * (incoming_edges[i].size() + outgoing_edges[i].size());
        return imp;
    }
    ...
    void computeInitialImportance () {
        for (int i = 0; i < N; ++i) {
            ...
            vertices[i].importance = get_importance(i) ;
    ...
    void dijkstra (int s, int maxD) {
        ...
        while (hops && !minPQ.empty()) {
            ...
            for (int i = 0; i < outgoing_edges[current].size(); ++i) {
                ...
                int next_cost = distF[current] + edge_cost;
